...
{
	i = 0;
	while (threadrun()) { SDL_Delay(1); updatescreen(); }
	setthreadrun(true);
	exit(0);
	return 0;
}
...
{
	if (!strcmp(argv[0]+strlen(argv[0])-4,".scr")) screensavercheck();
	if (!strcmp(argv[0]+strlen(argv[0])-4,".SCR")) screensavercheck();
    int mp = 0;
	...
	if (!isserver)
		if (argc > 1)
			for (int i = 1; i < argc; i++)
				if ((strstr (argv[i],"bs2addon://") == argv[i])) showgui = false;
	if (showgui)
	{
	...
	for (int i3 = 0; i3 < 10; i3++) nullparams[i3] = 0;
	...
	setVar("BSNIGHTLY",1);
	SDL_WM_SetCaption("Burning Sand 2 by Max Nagl (http://siebn.de)", "");
	SDL_WM_SetIcon(SDL_LoadBMP(checkfilename("icon.bmp")), NULL);


//	initsand(screen->w-getmenuwidth(MENU_BAR_LEFT)-getmenuwidth(MENU_BAR_RIGHT)-1,screen->h-getmenuwidth(MENU_BAR_TOP)-getmenuwidth(MENU_BAR_BOTTOM)-1);
    int z = 1;
    #ifdef COMPILER_PPC
	...
	while (!done)
	{
		...
		if ( (steps < 0) && (1000/minTickTime > speed->value) ) SDL_Delay(1);
		...
		while (SDL_PollEvent(&event))
		{
			...
			case SDL_MOUSEBUTTONUP:
					if (event.button.button == 4) findTrigger("MOUSEWHEELUP",0)->exec();
					if (event.button.button == 5) findTrigger("MOUSEWHEELDOWN",0)->exec();
				break;
			case SDL_VIDEORESIZE:
				{
					int width = event.resize.w+1; int height = event.resize.h+1;
		...
		if ( keystate[SDLK_LCTRL] ||  keystate[SDLK_RCTRL] ) ctrl->value = 1; else ctrl->value = 0;
		if ( keystate[SDLK_LSHIFT] ||  keystate[SDLK_RSHIFT] ) shift->value = 1; else shift->value = 0;
		if ( keystate[SDLK_LALT] ||  keystate[SDLK_RALT] ) alt->value = 1; else alt->value = 0;
		if ( kmods & KMOD_CAPS ) capslock->value = 1; else capslock->value = 0;
		if ( kmods & KMOD_NUM ) numlock->value = 1; else numlock->value = 0;
		if ( keystate[SDLK_LALT] ) lalt->value = 1; else lalt->value = 0;
		if ( keystate[SDLK_LSHIFT] ) lshift->value = 1; else lshift->value = 0;
		if ( keystate[SDLK_LCTRL] ) lctrl->value = 1; else lctrl->value = 0;
		if ( keystate[SDLK_RALT] ) ralt->value = 1; else ralt->value = 0;
		if ( keystate[SDLK_RSHIFT] ) rshift->value = 1; else rshift->value = 0;
		if ( keystate[SDLK_RCTRL] ) rctrl->value = 1; else rctrl->value = 0;
	}
