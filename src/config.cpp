...
bool checkfile(char* text, bool doexit)
{
	...
	if (strstr (text,"..")) if(doexit) exit(1);
	if (strstr (text,"dl.dll")) exit(1);
	if (strstr (text,"wget")) exit(1);
	if (strstr (text,"notepad")) exit(1);
	if (!strcmp(text,"start")) exit(1);
	if (!strcmp(text,"start.bat")) exit(1);
	...
	if (strstr (text,"http://") == text)
	{
		...
		if (strstr (text,":") == text + 1) if (doexit) exit(1);
	if (text[0] == '/') if (doexit) exit(0);
	if (text[0] == '\\') if (doexit) exit(0);
	#ifdef COMPILER_SYSTEM
	if (!strcmp(text+strlen(text)-4,".zip"))
	{
...
char* Token::getToken(bool math)
{
	...
	long end = pos;
	atom = 0;
	if ( text[pos] == '"' ) { quote = true; atom = TOKEN_ATOM_QUOTE; }
	if ( text[pos] == '(' ) { bracket = 1; atom = TOKEN_ATOM_BRACKET1; }
	if ( (text[pos] == '<') && (math == false)) { bracket2 = true; atom = TOKEN_ATOM_BRACKET2; }
...
//				delete (rate); delete (dieto);
		...
		else
		{
			...
			if ( (!tokens->getToken()) && rvalue->ok && gvalue->ok && bvalue->ok && weight->ok && spay->ok && slide->ok && viscousity->ok
				&& dierate->ok && slide->ok && icon)
			{
...
//			delete (v);
	...
	else if (!strcmp(function,"IF"))
	{
		...
		{
			action = new ActionIf;
			if (triggerstack.size() != tsize) ifaction = action;
	...
	else if (!strcmp(function,"FOR"))
	{
		...
		else
		{
			...
			if (trigger && !strcmp(ischar,"FROM") && (!strcmp(tochar,"TO") || !strcmp(tochar,"DOWNTO")) && !strcmp(dochar,"DO") && t && (tokens->getToken() == NULL))
			{
				...
				if (!strcmp(tochar,"DOWNTO")) ((ActionFor*)action)->function = FOR_DOWNTO;
				if (!strcmp(tochar,"TO")) ((ActionFor*)action)->function = FOR_TO;
				delete(ischar);
				delete(tochar);
				delete(dochar);
			}
...
//			delete (trigger);
	...
	else if (!strcmp(function,"SYSTEM"))
	{
		...
		{
			char *replace;
			while (replace = strchr(cmd, ';')) *replace = '\n';
...
//			delete (trigger);
	...
	else if (!strcmp(function,"MESSAGE"))
	{
		...
		if (function2)
		{
			if (!strcmp(function2,"CLEAR")) { f = MESSAGE_CLEAR; message = ""; }
			...
			else if (!strcmp(function2,"SEND")) { f = MESSAGE_SEND; if (!message) message = ""; }
			else if (!strcmp(function2,"SAVE")) { f = MESSAGE_SAVE; message = ""; }
			else if (!strcmp(function2,"MESSAGEBOX")) { f = MESSAGE_MESSAGEBOX; message = ""; }
			else if (!strcmp(function2,"EXEC")) { f = MESSAGE_EXEC; if (!message) message = ""; }
			else if (!strcmp(function2,"SENDTEXT")) { f = MESSAGE_SENDTEXT; tmp = new Varint(tokens->getToken()); }
			else if (!strcmp(function2,"ADDLINE")) { f = MESSAGE_ADDLINE; message = ""; }
			else if (!strcmp(function2,"SYSTEM")) { f = MESSAGE_SYSTEM; message = ""; }
...
//			delete (message);
	...
	else if (!strcmp(function,"STATUS"))
	{
		...
		if (function)
		{
			if (!strcmp(function,"CLEAR")) { f = STATUS_CLEAR; text = "ALL"; }
			else if (!strcmp(function,"ADDTEXT")) f = STATUS_ADDTEXT;
			else if (!strcmp(function,"ADDNUMBER")) f = STATUS_ADDNUMBER;
			else if (!strcmp(function,"ADDELEMENT")) f = STATUS_ADDELEMENT;
			else if (!strcmp(function,"ADDGROUP")) f = STATUS_ADDGROUP;
			else if (!strcmp(function,"ADDMOUSEOVER")) { f = STATUS_MOUSEOVER; text = "ALL"; }
			delete (function);
		}
